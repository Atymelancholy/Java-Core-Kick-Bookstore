...
public final class OrderCreateServlet extends BaseServlet {
    ...
    private static final int MAX_ORDER_QTY = 999;

    /** Logger. */
    private static final Logger LOG =
            LogManager.getLogger(OrderCreateServlet.class);
    ...
    protected void doPost(final HttpServletRequest req,
        ...
        try {
            ...
            app().orders().placeOrder(
                    user.id(), List.of(new OrderLine(productId, qty)));
            resp.sendRedirect(req.getContextPath() + "/app/orders");
        } catch (DomainException e) {
            req.getSession(true).setAttribute(
                    "flashProducts", e.messageKey());
            resp.sendRedirect(req.getContextPath() + "/app/products");
        } catch (NumberFormatException e) {
            req.getSession(true).setAttribute(
                    "flashProducts", "error.validation.product");
            resp.sendRedirect(req.getContextPath() + "/app/products");
        } catch (IllegalStateException e) {
            req.getSession(true).setAttribute(
                    "flashProducts", "error.order.stock");
            resp.sendRedirect(req.getContextPath() + "/app/products");
        } catch (Exception e) {
    ...
    private static int parseQty(final String raw) {
        int q = Integer.parseInt(raw == null ? "0" : raw.strip());
        if (q < 1 || q > MAX_ORDER_QTY) {
